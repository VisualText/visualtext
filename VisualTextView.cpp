...
//	edit.SetOptions(ECOOP_XOR,ECO_AUTOWORDSELECTION);
	...
	// NOTE: This code MUST FOLLOW doc->Update() to take effect! // SRP 010704
	// SET TAB STOPS
	edit.HideSelection(TRUE, FALSE);
	edit.SetSel(0, -1); // Full selection
	// USING SetTAbStops instead of this paraformat stuff.	// 09/05/08 AM.
	// Does not work for Vista, the "13" seems to be a hard-wired hack.	// 09/05/08 AM.
/*
	PARAFORMAT pf;
	...
	for( int itab = 0 ; itab < pf.cTabCount  ; itab++ )
		pf.rgxTabs[itab] = (itab + 1) * ((13 * appIntRuleTabSize * m_intCharWidth) + 4);
	edit.SetParaFormat(pf);
*/
	edit.SetSel(0, 0);
	edit.HideSelection(FALSE, FALSE);

	// TAB SIZE ON VISTA IS WRONG.                     // 09/05/08 AM.
	// Use MFC code example for CEditView\SetTabStops. // 09/05/08 AM.
	CEdit* pCtrl = (CEdit*)this;								// 09/05/08 AM.
	pDC = GetParentFrame()->GetDC();							// 09/05/08 AM.
	SetTabStops(pCtrl,pDC);					               // 09/05/08 AM.
	GetParentFrame()->ReleaseDC(pDC);						// 09/05/08 AM.
...
// TAB SIZE ON VISTA IS WRONG.                        // 09/05/08 AM.
// Use MFC code example for CEditView\SetTabStops.    // 09/05/08 AM.
bool CVisualTextView::SetTabStops(					      // 09/05/08 AM.
	CEdit *pCtrl,
	CDC *pDC
	)
{ 

	// TAB SIZE ON VISTA IS WRONG.
	// Use MFC code example for CEditView\SetTabStops.
//	CEdit* pCtrl = (CEdit*)GetFocus();
	...
	MapDialogRect(pCtrl->m_hWnd,rect);

	// This general solution doesn't work right on XP (haven't tried on Vista.)
	// Gives a double-wide tab for the default font.	// 09/05/08 AM.
	int tabsize = ((4 * tm.tmAveCharWidth * 100) / rect.Width());

	// HACK.	GET THE RIGHT TAB SIZE FOR VISTA.
   ...
   osvi.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEX);
	...
	bool bVistaOrLater = 
		...
		(osvi.dwMajorVersion > 5) );	// Vista, Longhorn, or later.
//		((osvi.dwMajorVersion == 5) && (osvi.dwMinorVersion > 0));	// Reference code.
   ...
   // If that fails, try using the OSVERSIONINFO structure.
	...
	if (bVistaOrLater)
		{
		pCtrl->SetTabStops(16);	// THIS WORKS FOR VISTA.
		return true;
		}
...
//	pCtrl->SetTabStops(tabsize);	// Doesn't work.

	pCtrl->SetTabStops(16);	// THIS WORKS FOR XP.
